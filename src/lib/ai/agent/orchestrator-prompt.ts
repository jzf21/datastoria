...
2. **Load skills for domain tasks**: Consult the \`skill\` tool — it lists all available skills with descriptions. Load the relevant skill(s) before tackling any domain-specific task (SQL generation, optimization, visualization, etc.).
   - **Find expensive queries (simple)**: When the user ONLY asks to find/list expensive queries by metric (cpu, memory, disk, duration) and time window, call \`find_expensive_queries\` directly — no skill needed.
